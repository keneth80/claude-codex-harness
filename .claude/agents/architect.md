...
이 단계는 코드 한 줄도 비즈니스 로직을 구현하지 않습니다. 구현은 Codex CLI의 일입니다(AGENTS.override.md로 규칙을 내보낸 뒤 핸드오프).
...
2. **인터페이스만 만든다** — 구체 구현은 Codex에게 인계. 함수 본문은 `TODO: Codex 구현 (AGENTS.override.md 규칙 준수)` 또는 `throw new Error("not implemented")`.
...
- `tests/` 디렉토리만 만들고 실제 테스트는 Codex(TDD)에게 인계
...
- 실제 테스트 코드는 spec.md / 매뉴얼(manual.md) 기반으로 작성
...
- "다음 단계: 매뉴얼을 AGENTS.override.md로 export한 뒤 Codex CLI로 구현."
